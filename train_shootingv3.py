...
def get_reward(state,state_,flag):
    for i in range(teamA_num):
        ...
        if np.linalg.norm(player - soccer) > np.linalg.norm(player_ - soccer_) :
            r += 1
        else :
            r -= 1
        if np.linalg.norm(soccer - [field_length/2,0]) > np.linalg.norm(soccer_ - [field_length/2,0]):
            r += 3
        r += 50*flag
        reward = r
...
def main():
    ...
    agentA = DDPG(alpha=0.0001, beta=0.001, state_dim=4*(teamA_num+teamB_num+1),
                 ...
                 critic_fc1_dim=512, critic_fc2_dim=256, critic_fc3_dim=128, ckpt_dir=args.checkpoint_dir + '03' + '/',
                 batch_size=64)
    create_directory(args.checkpoint_dir + '03' + '/',

...
def get_reward(state,state_,flag):
    for i in range(teamA_num):
        ...
        reward = -5 * cos 
        reward += np.linalg.norm([player[0] - soccer[0],player[1]-soccer[1]]) - np.linalg.norm(x1)
        reward += 2 * (np.linalg.norm([gate[0]-soccer[0],gate[1]-soccer[1]]) - np.linalg.norm(x2))
...
def main():
    ...
    agentA = DDPG(alpha=0.0001, beta=0.001, state_dim=4*(teamA_num+teamB_num+1),
                 ...
                 critic_fc1_dim=512, critic_fc2_dim=256, critic_fc3_dim=128, ckpt_dir=args.checkpoint_dir + '07' + '/',
                 batch_size=64)
    create_directory(args.checkpoint_dir + '07' + '/',
